...
| **Hermes Agent** | https://github.com/NousResearch/hermes-agent | Persistent agent loop, messaging gateways, memory, skills, toolsets, MCP, channel continuity, tool-call persistence | Telegram/Discord/Slack-style channels, skills loading, memory integration, persistent agent behavior, gateway design | One agent core across channels; channels are adapters at the edge. Preserve working team/subagent features, but keep parent/session ownership explicit and do not let gateway or memory become a second authority. |
| **OpenHands Agent Canvas** | https://github.com/OpenHands/OpenHands | Frontend/control center, backend selection, user-facing orchestration | UI/control-center architecture and how clients select/connect to agent backends | Do not use this repo as runtime implementation truth: current agent/tool/conversation/workspace ownership lives in `OpenHands/software-agent-sdk`. |\n| **OpenHands software-agent-sdk** | https://github.com/OpenHands/software-agent-sdk | Agent runtime, actions/observations, conversations, workspaces, events, confirmation policy, Agent Server | Runtime implementation, action/observation flow, workspace abstraction, lifecycle states, backend API ownership | This is the canonical current OpenHands runtime implementation reference. Adapt narrow contracts rather than importing the whole runtime. |
...
| **Gemini CLI** | https://github.com/google-gemini/gemini-cli | Browser-specialized agent, browser/CDP lifecycle, isolated browser tools, tool scheduler, policy/confirmation, Gemini-native CLI | Browser-agent design, browser session/profile ownership, dynamic tool discovery, centralized tool state/policy/confirmation | Primary browser-agent implementation reference. Preserve its architectural seams, not Gemini-specific model assumptions. |\n| **Google Antigravity CLI** | https://github.com/google-antigravity/antigravity-cli | Terminal-agent UX, status projection, persistent history, artifacts, permissions, subagents, background work, Remote Control | TUI/statusline/review/subagent/remote-control patterns where exact product behavior matters | Public repo is partial; use official product docs for product behavior and repo source only where present. |
| **Claude Code** | https://github.com/anthropics/claude-code | CLI, tool/permission vocabulary, hooks, skills/plugins, MCP, subagents/teams, background agents, worktrees, headless/remote-control | Project instruction scoping, hook lifecycle, permission surfaces, subagent/team and background-agent UX | Public repo is not a complete production-core mirror; anchor core product behavior to official Claude Code docs. |
| **OpenCode** | https://github.com/anomalyco/opencode | CLI/TUI/desktop, provider/model registry, permission-gated tools, agents/subagents, sessions, MCP, plugins, LSP, remote attach/web | Registry separation, session/child-session projection, remote surfaces, TUI/runtime configuration separation | Reuse seams; do not duplicate target authorization/execution/persistence/completion authority. |
| **DeepSeek Harness** | https://github.com/deepseek-ai/deepseek-harness | Explicit harness seams, agent loop, ordered tool scheduling, browser/computer capability registration, subagent lifecycle, skills | Harness architecture, browser/computer capability ownership, deterministic agent/tool lifecycle, explicit subagent control | Use the harness to make ownership explicit. Do not fragment a working target owner merely because a plugin seam exists upstream. |
...
For Brew or another agent runtime (including systems with working teams/subagents):
